...
def asigna_entregas(casas_df, centros_df):
    # INPUT
    # casas_df.- DataFrame
    #  ReporteId.- Etiquetas de los datos
    #  lon.- Longitud
    #  lat.- Latitud
    # centro_df.- DataFrame
    ...
    casas, centros = asigna_medias(
        casas_df.rename(columns={'ReporteId': 'labs_x'}),
    ...
    for c in asignaciones.labs_centros.unique():
        ...
        X = (asignaciones.
             query('labs_centros=={}'.format(c)).
             assign(labs_x=lambda x: x.ReporteId.tolist()))
        medias = pd.DataFrame({
            'labs_meds': range(n),
...
if __name__ == '__main__':
    ...
    ReporteId, labs_centros, labs_flotilla = asigna_entregas(
        casas_df, centros_df)
